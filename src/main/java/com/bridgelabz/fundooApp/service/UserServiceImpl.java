...
public class UserServiceImpl implements UserService 
{
	...
	{
		boolean ismail = userRepository.findByEmail(userDto
				.getEmail()).isPresent();
		if (!ismail) 
		{
			...
			try 
			{
				...
				String token = tokenGenerator.generateToken(savedUser
						.getUserid());
				String activationUrl = getLink(requestUrl, 
						"/verification/", token);
				Email email = new Email();
				email.setTo("iamali74111@gmail.com");
				email.setSubject("Account verification");
				email.setBody("Please verify your email id "
						+ "by using below link \n" + activationUrl);
	...
	{
		Optional<User> optUser = userRepository.findByEmail(loginDto
				.getEmail());
		if (optUser.isPresent())
		{
			...
			try 
			{
				if (EncryptUtil.isPassword(loginDto, user))
				{
					...
					{
						throw new UserException("please verify "
								+ "your email");
					}
				} 
				else 
			...
			StringBuffer requestUrl) 
	{
		...
		if (optUser.isPresent()) 
		{
			...
			try 
			{
				...
				String resetUrl = getLink(requestUrl, 
						"/resetpassword/", token);
				Email email = new Email();
				email.setTo("iamali74111@gmail.com");
	...
	private String getLink(StringBuffer requestUrl, String mappingUrl,
			String token)
	{
		String url = requestUrl.substring(0, requestUrl
				.lastIndexOf("/")) + mappingUrl + token;
		return url;
	}
}
